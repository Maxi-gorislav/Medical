...
class PrestoChangeoContext
{
	...
	public function __construct()
	{
		...
		$this->controller = new PrestoChangeoControllerBackwardModule();
		if (is_object($cookie))
		{
			...
			$this->customer = new PrestoChangeoCustomerBackwardModule((int)$cookie->id_customer);
		...
		$this->shop = new PrestoChangeoShopBackwardModule();
	}
	...
	public static function shop()
	{
		if (!self::$instance->shop->getContextType())
			return PrestoChangeoShopBackwardModule::CONTEXT_ALL;
 ...
 */
class PrestoChangeoShopBackwardModule extends Shop
{
	const CONTEXT_ALL = 1;
 ...
 */
class PrestoChangeoControllerBackwardModule
{
	/**
	 * @param $js_uri
	 * @return void
 ...
 * Allow to use method declared in 1.5
 */
class PrestoChangeoCustomerBackwardModule extends Customer
{
	public $logged = false;
	/**
	...
	{
		global $cookie;
		$this->logged = is_object($cookie) && method_exists($cookie, 'isLogged')?$cookie->isLogged():false;
	}
	...
	public function isLogged($with_guest = false)
	{
		...
		if ($cookie->isLogged() == 1 && $this->id && Validate::isUnsignedId($this->id) && Customer::checkPassword($this->id, $this->passwd))
			return true;
		return false;
	}
