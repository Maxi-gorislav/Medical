...
var Gmc = function(sName){
	...
	this.generateDataFeed = function(aParams) {
		...
		$.ajax({
			type : 'POST',
			url : sURI,
			data : sParams + '&iShopId='+iShopId+'&sFilename='+sFilename+'&iLangId='+iLangId+'&sLangIso='+sLangIso+'&sCountryIso='+sCountryIso+'&iFloor='+iStep+'&iTotal='+iTotal+'&iProcess='+iProcess+'&bReporting='+bReporting,
			dataType : 'json',
			async : true,
			success: function(data) {
				// use case - no error
				if (data.status == 'ok') {
					...
					// use case - finalize the recursive ajax query
						...
						if (bReporting) {
							$(sDisplayReporting).attr('href', $(sDisplayReporting).attr('href') + '&lang=' + sLangIso.toLowerCase() +'_'+sCountryIso.toUpperCase()+'&count='+data.process);
