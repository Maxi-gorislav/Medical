<?php
/**
 * install-config_class.php file defines InstallTab class to install / uninstall module configuration
 */
		...
		if (defined('_GMC_LOG_JAM_CONFIG') && _GMC_LOG_JAM_CONFIG) {
			...
			foreach ($GLOBALS[_GMC_MODULE_NAME . '_TABS'] as $sAdminClassName => $aTab) {
				...
				if (file_exists(_PS_MODULE_DIR_ . $oTab->module . _GMC_PATH_VIEWS . _GMC_PATH_IMG . _GMC_TPL_ADMIN_PATH . $sAdminClassName . '.gif')) {
					@copy(_PS_MODULE_DIR_ . $oTab->module . _GMC_PATH_VIEWS . _GMC_PATH_IMG . _GMC_TPL_ADMIN_PATH . $sAdminClassName . '.gif', _PS_IMG_DIR_ . 't/' . $sAdminClassName . '.gif');
				}
