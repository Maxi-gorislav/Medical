<?php
/**
 * xml-strategy_class.php file defines method to manage XML files
 */
...
final class BT_XmlStrategy
{
	...
	/**
	 * loadProduct() method load Products for XML
	 *
	 */
	public function loadProduct()
	{
		// get currency ISO
		$sCurrencyIso = $GLOBALS[_GMC_MODULE_NAME . '_AVAILABLE_COUNTRIES'][$this->aParams['sLangIso']][$this->aParams['sCountryIso']]['currency'];

		// set different vars required to calculate some things
		$this->data->currencyId = Currency::getIdByIsoCode($sCurrencyIso);
		...
		if (!empty(GMerchantCenter::$aConfiguration['GMERCHANTCENTER_SHIP_CARRIERS'][Tools::strtoupper($this->aParams['sCountryIso'])])) {
			$this->data->currentCarrier = new Carrier((int)GMerchantCenter::$aConfiguration['GMERCHANTCENTER_SHIP_CARRIERS'][Tools::strtoupper($this->aParams['sCountryIso'])]);
		}
		...
		if (!empty(GMerchantCenter::$bCompare15)) {
			...
			$cookie->id_country = $this->data->countryId;
			$cookie->id_currency = $this->data->currencyId;
		}
