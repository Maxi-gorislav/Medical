    ...
    <div class="box box-small cleafix">
        ...
        {if $empty eq 0}
            {foreach from=$fields_data item=field_data}
                <p>
                    <strong class="dark">{l s={$field_data['field_label']|escape:'htmlall':'UTF-8'} mod='supercheckout'}:</strong>
                    <span class="color-myaccount">{$field_data['field_value']|escape:'htmlall':'UTF-8'}</span>
                </p>
            {/foreach}
        {else}
