...
$(document).ready(function() {
    ...
    if (typeof guest_information != 'undefined') {
        guest_information = JSON.parse(guest_information);
    }
...
//to hide social login block when by default guest checkout is selected
    ...
    $('.supercheckout_shipping_option').live('change', function() {
        ...
        // added by Gaurav pal january 14 2016 for gls shipping method
        if (typeof glsCarrierRelayId != 'undefined') {
            var idCarrierSelected = $(this).val().slice(0, -1);
            if (idCarrierSelected == glsCarrierRelayId) {
                $("#pickup-info").html('');
                if (typeof getPreviouslyChoosenRelay == 'function') {
    ...
    if ((typeof formatedAddressFieldsValuesList != 'undefined') && formatedAddressFieldsValuesList != null) {
        ...
        for (var i in formatedAddressFieldsValuesList[id_address]['ordered_fields']) {
            ...
            if (reg.test(field)) {
                ...
                var values = '';
                for (var j in keys) {
                    if (formatedAddressFieldsValuesList[id_address]['formated_fields_values'][keys[j]] != null)
                        values += formatedAddressFieldsValuesList[id_address]['formated_fields_values'][keys[j]] + ' ';
                }
                html += '<span>' + values + '</span>';
            } else {
                if (formatedAddressFieldsValuesList[id_address]['formated_fields_values'][field] != null)
...
function loadCarriers() {
    ...
    $.ajax({
        ...
        success: function(jsonData)
        {
            ...
            if (typeof jsonData['HOOK_BEFORECARRIER'] != 'undefined')
...
function updateQty(element, up, down) {
    $('#cart_update_warning .permanent-warning').remove();
                ...
                $.ajax({
                    ...
                    success: function(jsonData) {
                        if (jsonData.hasError) {
                            ...
                            var update_qty = parseInt($('input[name=' + element + '_hidden]').val());
                            if (up == 1) {
                                $('input[name=' + element + ']').val(update_qty + 1);
                                $('input[name=' + element + '_hidden]').val(update_qty + 1);
                            }
                            else if (down == 1) {
                                $('input[name=' + element + ']').val(update_qty - 1);
                                $('input[name=' + element + '_hidden]').val(update_qty - 1);
                                ...
                                $('input[name=' + element + ']').val(update_qty);
                        } else {
                            $.gritter.add({
                                title: notification,
    ...
    for (i in product_list)
    {
       ...
       if (reduction && typeof(initial_price) !== 'undefined')
       {
           if (reduction_type == 'amount')
                   priceReductionPercent = product_list[i].reduction_formatted;
           else
           {
                   var display_price = 0;
                   ...
                   priceReductionPercent = ps_round((product_list[i].price_without_quantity_discount - display_price)/product_list[i].price_without_quantity_discount * -100);
           }
       }
        ...
        if (priceDisplayMethod !== 0)
        {
            $('span[id^=product_price_' + key_for_blockcart+']').html('<span class="price">' + current_price + '</span>' + initial_price_text);
            if (typeof (product_list[i].customizationQuantityTotal) !== 'undefined' && product_list[i].customizationQuantityTotal > 0)
                $('td[id^=total_product_price_' + key_for_blockcart+']').html(formatCurrency(product_list[i].total_customization, currencyFormat, currencySign, currencyBlank));
            else
                $('td[id^=total_product_price_' + key_for_blockcart+']').html(formatCurrency(product_list[i].total, currencyFormat, currencySign, currencyBlank));
            if (product_list[i].quantity_without_customization != product_list[i].quantity)
                $('td[id^=total_product_price_' + key_for_blockcart_nocustom+']').html(formatCurrency(product_list[i].total, currencyFormat, currencySign, currencyBlank));
        }
        else
        {
            $('span[id^=product_price_' + key_for_blockcart+']').html('<span class="price">' + current_price + '</span>' + initial_price_text);
            if (typeof (product_list[i].customizationQuantityTotal) !== 'undefined' && product_list[i].customizationQuantityTotal > 0)
                $('td[id^=total_product_price_' + key_for_blockcart+']').html(formatCurrency(product_list[i].total_customization_wt, currencyFormat, currencySign, currencyBlank));
            else
                $('td[id^=total_product_price_' + key_for_blockcart+']').html(formatCurrency(product_list[i].total_wt, currencyFormat, currencySign, currencyBlank));
            if (product_list[i].quantity_without_customization != product_list[i].quantity)
                $('td[id^=total_product_price_' + key_for_blockcart_nocustom+']').html(formatCurrency(product_list[i].total_wt, currencyFormat, currencySign, currencyBlank));
        }
        ...
        $('input[name^=quantity_' + key_for_blockcart_nocustom + ']:hidden').val(product_list[i].id_customization ? product_list[i].quantity_without_customization : product_list[i].cart_quantity);
        if (typeof (product_list[i].customizationQuantityTotal) !== 'undefined' && product_list[i].customizationQuantityTotal > 0)
            $('span[id^=cart_quantity_custom_' + key_for_blockcart+']').html(product_list[i].customizationQuantityTotal);
...
function actionOnPaymentSelect(element) {
    ...
    if (!($.inArray(payment_module_name,['paypalpro' ,'stripejs' , 'mdstripe','firstdata' ,'migs' ,'conektatarjeta' ,'stripepro' ,'braintreejs_backup' ,'twocheckout' ,'brinkscheckout' ,'ewayrapid' ,'npaypalpro' ,'authorizeaim'])!= -1)) {
        $('#selected_payment_method_html').html(''); // to hide form if customer select any other payment method later
    }
    if ($.inArray(payment_module_name,['librapay' ,'codpro' ,'bankwirepro' ,'cashonpickup' ,'dibs' ,'klarnaofficial' ,'ebs' ,'virtualmerchant' ,'lc_paywithfee' ,'klikandpay' ,'transferuj' ,'eanpayment' ,'prepayment' ,'ecm_privat24' ,'xpay' ,'ecm_liqpay' ,'bankmandiri' ,'bankbjb' ,'bankbnisyariah' ,'bankbrisyariah' ,'bankmuamalat' ,'bankbca' ,'bankbsm' ,'quote' ,'magasin' ,'payshop' ,'eupago_multibanco' ,'paysonCheckout2' ,'eupago_mbway' ,'opartmultipaybycheck' ,'banklinkmaksekeskus', 'redsys', 'realexredirect', 'cardpay', 'visacuotas2', 'bitpay', 'paytm', 'cashondelivery', 'barion','CSPayBinder', 'cashondeliveryplus', 'mollie_ideal' ,'mpco' ,'mollie_mistercash' ,'mollie_belfius',  'mollie_bitcoin', 'mollie_banktransfer', 'mollie_sofort' ,'mollie_paypal' ,'payeer' ,'western' ,'bankwirepluss' ,'authorizedotnet' ,'secureframe' ,'compropago' ,'checkout' ,'westernunion' ,'billmateinvoice' ,'billmatepartpayment' ,'mercadoc' ,'boletosantanderpro' ,'payu' ,'payulatam' ,'zipcheck' ,'megareembolso' ,'payinstore' ,'codfee' ,'finanziamento' ,'megashoppay'])!= -1) {
    ...
    } else if ($.inArray(payment_module_name,['bankwire' ,'boleto' ,'invoicepayment' ,'pagofacil' ,'postepay' ,'paysera' ,'offlinecreditcard' ,'trustly' ,'cheque' ,'deluxecodfees'])!=-1) {
        getPaymentForm(element);
    } else if ($.inArray(payment_module_name,['stripejs' , 'mdstripe','cmcic' ,'stripepro' ,'paypalpro' ,'firstdata' ,'migs' ,'conektatarjeta' ,'braintreejs_backup' ,'twocheckout' ,'brinkscheckout' ,'ewayrapid' ,'npaypalpro' ,'mobilpay_cc' ,'authorizeaim' ,'khipupayment' ,'paynl_paymentmethods' ,'mollie' ,'quickpay' ,'custompaymentmethod' ,'cofidis' ,'mpar' ,'payonlinecc' ,'billmategateway' ,'moneybookers' ,'mercadopagobr' ,'djtalmoip' ,'atos' ,'payline' ,'winbnk' ,'faspay' ,'paynlpaymentmethods' ,'add_gopay_new' ,'banc_sabadell' ,'be2bill' ,'py_atos' ,'payway' ,'payfortfort' ,'hellaspay' , 'liisi', 'pdpaypal', 'tpvredsyscajamar', 'paypal', 'paypalstandard', 'ggbank' ,'paypalwithfee' ,'clickcanarias' ,'mobilpay_cc' ,'parspalpayment' ,'pronesis_bancasella' ,'paypalmx' ,'cmcic_tbweb' ,'sisoweb' ,'citrus' ,'banc_sabadell' ,'ccavenue' ,'ogone' ,'epay' ,'creditcardpaypal' ,'paypalusa' ,'sisowideal' ,'sisowmc'])!=-1) {
    ...
    $.ajax({
        ...
        success: function(json)
        {
            ...
            if (payment_module_name == 'authorizeaim') {
                ...
                $('#asubmit').hide();
            } else if ($.inArray(payment_module_name,['stripejs', 'mdstripe','stripepro' ,'paypalpro' ,'firstdata' ,'migs' ,'conektatarjeta' ,'braintreejs_backup' ,'brinkscheckout' ,'twocheckout'])!=-1) {
                //$('.velsof_content_section').appendTo('#selected_payment_method_html');
                $('#selected_payment_method_html').html($('.velsof_content_section').html());
                $('#firstdata_submit').hide();
                $('a#click_migs').trigger('click');
                ...
                $('#pppro_submit').hide();
                if (payment_module_name == 'mdstripe')
            ...
            } else if ($.inArray(payment_module_name,['cofidis' ,'custompaymentmethod' ,'payonlinecc', 'billmategateway' ,'moneybookers' ,'djtalmoip' ,'atos' ,'payline' ,'winbnk' ,'faspay' ,'paynlpaymentmethods' ,'add_gopay_new' ,'quickpay' ,'mollie' ,'paynl_paymentmethods'])!=-1) {
            ...
            else if (payment_module_name == 'mercadopagobr') {
                $('#velsof_payment_dialog .velsof_action_section').hide();
                $('#velsof_payment_container .velsof_content_section').css('height', '540px');
                $('#supercheckout_dialog_proceed').hide();
            }
            if (proceed_to_payment) {
                moveToPayment();
                    ...
                    if (extrasError) {
                        ...
                        if (is_free_order) {
                            ...
                            if ($('input:radio[name="payment_method"]:checked').length) {
                                ...
                                if ($.inArray(p_m_name,['isicmcic' ,'stripejs' , 'mdstripe','stripepro' ,'paypalpro' ,'firstdata' ,'migs' ,'conektatarjeta' ,'braintreejs_backup' ,'twocheckout' ,'brinkscheckout' ,'ewayrapid' ,'npaypalpro' ,'authorizeaim' ,'librapay' ,'secureframe' ,'bankwirepro' ,'codpro' ,'cashonpickup' ,'dibs' ,'klarnaofficial' ,'bankwireplus' ,'ebs' ,'virtualmerchant' ,'cs_cardsave' ,'lc_paywithfee' ,'klikandpay' ,'transferuj' ,'eanpayment' ,'prepayment' ,'ecm_privat24' ,'xpay' ,'ecm_liqpay' ,'bankmandiri' ,'bankbjb' ,'bankbnisyariah' ,'bankbrisyariah' ,'bankmuamalat' ,'bankbca' ,'bankbsm' ,'eupago_mbway' ,'eupago_multibanco' ,'quote' ,'payshop' ,'magasin' ,'paysonCheckout2' ,'opartmultipaybycheck' ,'banklinkmaksekeskus', 'redsys', 'realexredirect', 'cardpay', 'visacuotas2', 'bitpay', 'paytm', 'cashondelivery', 'barion','cashondeliveryplus', 'CSPayBinder' ,'mollie_ideal' ,'mpco' ,'mollie_mistercash' ,'mollie_belfius', 'mollie_bitcoin', 'mollie_banktransfer', 'mollie_sofort' ,'mollie_paypal' ,'western' ,'payeer' ,'authorizedotnet' ,'compropago' ,'checkout' ,'westernunion' ,'billmatepartpayment' ,'billmateinvoice' ,'paysondirect' ,'mercadoc' ,'boletosantanderpro' ,'payu' ,'megashoppay' ,'zipcheck' ,'megareembolso' ,'payinstore' ,'codfee' ,'obsredsys' ,'hipay' ,'psphipay' ,'finanziamento' ,'cecatpv' ,'dineromail' ,'payulatam' ,'cuatrob' ,'robokassa'])!=-1) {
                                    moveToPayment();
                                } else {
                                    actionOnPaymentSelect($('input:radio[name="payment_method"]:checked'));
                                }
...
function moveToPayment() {
    ...
    if ($(dialogContainer + '#payment_fetch_error').length) {
        ...
        if (p_m_name == 'isicmcic') {
            location.href = $("input[type=hidden]", $('input:radio[name="payment_method"]:checked').parent()).attr('value');
        }
        else if (p_m_name == 'paypal' && url == 'javascript:void(0)') {
            $('#paypal_payment_form_payment').submit(); // above statement was not working for Prestashop 1.6.1.0
        } else if(p_m_name == 'liisi'){
            doAjax(event, 'liisi');return false;
        } else if(p_m_name == 'paypalstandard'){
            $('#paypal_standard_button_container form').submit();
        } else if(p_m_name == 'tpvredsyscajamar'){
            $('#redsys_cajamar_form').submit();
        } else if(p_m_name == 'pdpaypal'){
            $('#paypal_payment_form').submit();
        } else if (p_m_name == 'paypalwithfee') {
            $('#paypal_payment_form1').submit();
        } else if (p_m_name == 'payfortfort') {
            $('#payfortpaymentform input[type=submit]').trigger('click');
        } else if (p_m_name == 'hellaspay') {
            $('#hellaspay_payment_form').submit();
        } else if (p_m_name == 'ggbank') {
            $('#ggbank_form').submit();
        } else if (p_m_name == 'clickcanarias') {
            $('#clickcanarias_form').submit();
        } else if (p_m_name == 'payway') {
            $('#paywayForm').submit();
        }
        ...
        } else if (p_m_name == 'py_atos') {
            sendATOS(1);
        } else if (p_m_name == 'be2bill') {
            $('#be2bill_payment_form').submit();
        } else if (p_m_name == 'banc_sabadell') {
            $('#SabadellTPVForm').submit();
        }
        ...
        } else if (p_m_name == 'payline') {
            createPaymentPop();
            $('#velsof_payment_dialog').hide();
            $('p.box.payment_module.payline_payment').trigger('click');
        } else if (p_m_name == 'winbnk') {
            createPaymentPop();
            $('#velsof_payment_dialog').hide();
            $('.exclusive_large').trigger('click');
        } else if (p_m_name == 'atos') {
            createPaymentPop();
            $('#velsof_payment_dialog').hide();
            $('div.atos form input[type="IMAGE"]').trigger('click');
        } else if (p_m_name == 'djtalmoip') {
            createPaymentPop();
            $(document).on('click', '.moipay .pchoice', function() {
                $('.moipay .pchoice').removeClass('active-choice');
                $(this).addClass('active-choice');
            });
            if (typeof MoipPagamentos == 'function') {
                MoipPagamentos();
            }

        } else if ($.inArray(p_m_name,['khipupayment' ,'paynl_paymentmethods' ,'mollie' ,'custompaymentmethod' ,'cofidis' ,'payonlinecc' ,'moneybookers' ,'mercadopagobr' ,'faspay' ,'paynlpaymentmethods' ,'epay' ,'quickpay'])!=-1) {
            createPaymentPop();
            if (p_m_name != 'custompaymentmethod' && p_m_name != 'payonlinecc')
                $('#velsof_payment_dialog .velsof_action_section').show(); // @nitin 27 July - Important to show proceed button if it is hide in previous payment method selection	    
        } else if (p_m_name == 'billmategateway') {
            createPaymentPop();
	    $('#velsof_payment_container div.velsof_content_section').css('height', 'auto');
	    $('#velsof_payment_container div.velsof_content_section').css('overflow-y', 'auto');
	    $('#velsof_payment_container div.velsof_content_section div.row').css('margin', 'auto');
        } else if (p_m_name == 'mpar') {
            createPaymentPop();
            $('#velsof_payment_dialog .velsof_action_section').hide();
            $('#botonMP').click(function() {
                hide_progress();
                $('#velsof_payment_dialog').hide();
            });
            ...
            location.href = $('#' + $('input:radio[name="payment_method"]:checked').attr('value') + '_name').val();
        } else if ($.inArray(p_m_name,['bankwire' ,'mercadopago' ,'add_bankwire', 'multibankwire' , 'nimblepayment', 'aplazame', 'robokassa', 'egrpaypalpro', 'agilepaypal','cielows' ,'bbslip' ,'nimblepayment' ,'multisafepayamex' ,'multisafepaymaestro' ,'multisafepaysofort' ,'multisafepayvisa' ,'multisafepaymastercard' ,'mercanet' ,'gestpay' ,'GPWebPayBinder' ,'pscodfee' ,'bluesnap' ,'koin' ,'payuc' ,'lc_paywithfee' ,'lc_paywithfee' ,'add_dobirka_fee' ,'postfinancecw_postfinanceefinance' ,'postfinancecw_creditcard' ,'postfinancecw_postfinancecard' ,'lemonway' ,'sagepay' ,'SagePay' ,'klarnaofficial' ,'dibs' ,'ebs' ,'cashonpickup' ,'cpay' ,'cminvoicepay' ,'swipp' ,'boleto' ,'pstransparenteloja5' ,'spsbradesco' ,'cielows' ,'edinar' ,'clictopay' ,'allpay' ,'pay2go' ,'cash' ,'postfinance' ,'pagseguro' ,'braintreejs' ,'bcash' ,'invoicepayment' ,'przelewy24' ,'prestalia_cashondelivery' ,'virtpaypayment' ,'cashondeliveryfeeplus' ,'pagonlineimprese' ,'mokejimai' ,'payplug' ,'seurcashondelivery','universalpay' ,'mandiri' ,'bni' ,'bca' ,'veritranspay' ,'przelewy24' ,'transbancaria' ,'cashondeliveryplusmax' ,'multibanco' ,'ceca' ,'dotpay' ,'postepay' ,'paypaladvanced' ,'trustly' ,'billmateinvoice' ,'billmatepartpayment' ,'cheque' ,'westernunion' ,'paysondirect' ,'mercadoc' ,'boletosantanderpro' ,'payu' ,'librapay' ,'secureframe' ,'codpro' ,'bankwirepro' ,'cashonpickup' ,'klarnaofficial' ,'dibs' ,'bankwireplus' ,'ebs' ,'virtualmerchant' ,'lc_paywithfee' ,'klikandpay' ,'transferuj' ,'eanpayment' ,'prepayment' ,'ecm_privat24' ,'xpay' ,'ecm_liqpay' ,'bankbsm' ,'bankbca' ,'bankmuamalat' ,'bankbrisyariah' ,'bankbnisyariah' ,'bankbjb' ,'bankmandiri' ,'quote' ,'magasin' ,'payshop' ,'eupago_mbway' ,'eupago_multibanco' ,'paysonCheckout2' ,'opartmultipaybycheck' ,'banklinkmaksekeskus', 'redsys', 'realexredirect', 'cardpay', 'visacuotas2', 'bitpay', 'paytm', 'cashondelivery', 'barion', 'cashondeliveryplus', 'CSPayBinder','mollie_ideal' ,'mpco' ,'mollie_mistercash' ,'mollie_belfius',  'mollie_bitcoin', 'mollie_banktransfer', 'mollie_sofort' ,'mollie_paypal' ,'payeer' ,'western' ,'authorizedotnet' ,'compropago' ,'checkout' ,'megashoppay' ,'payulatam' ,'zipcheck' ,'megareembolso' ,'deluxecodfees' ,'payinstore' ,'codfee' ,'obsredsys' ,'hipay' ,'psphipay' ,'finanziamento'])!=-1) {
        ...
        } else if (p_m_name == 'cs_cardsave') {
            $('#cs_cardsave_FORM').submit();
        ...
        } else if (p_m_name == 'paypalpro') {
            hide_progress(); // to hide progress bar in case some error occur in first data payment form
            $('#velsof_payment_container .velsof_content_section').html('');
            $('#pppro_submit').trigger('click');
        } else if (p_m_name == 'migs') {
            hide_progress(); // to hide progress bar in case some error occur in first data payment form
            ...
            } else if($(dialogContainer+'.payment_module a').length){
                location.href = $(dialogContainer+'.payment_module a').attr('href');
...
function confirmOrder() {
    var payment_module_name = $('input:radio[name="payment_method"]:checked').attr('id');
    var dialogContainer = '#velsof_payment_dialog .velsof_content_section ';
    if ($.inArray(payment_module_name,['bankwire','invoicepayment'])!=-1) {
        ...
        document.getElementById('gopay-payment-form').submit();
        return false;
    }
    if ($.inArray(payment_module_name,['bankwire' ,'mercadopago' ,'add_bankwire', 'multibankwire', 'nimblepayment', 'aplazame', 'robokassa', 'egrpaypalpro', 'agilepaypal','bbslip' ,'cielows' ,'nimblepayment' ,'multisafepaymastercard' ,'multisafepayvisa' ,'multisafepaysofort' ,'multisafepaymaestro' ,'multisafepayamex' ,'mercanet' ,'gestpay' ,'pscodfee' ,'GPWebPayBinder' ,'bluesnap' ,'koin' ,'payuc' || p_m_name == 'lc_paywithfee' ,'add_dobirka_fee' ,'postfinancecw_postfinanceefinance' ,'postfinancecw_creditcard' ,'postfinancecw_postfinancecard' ,'lemonway' ,'SagePay' ,'sagepay' ,'klarnaofficial' ,'dibs' ,'ebs' ,'cpay' ,'cminvoicepay' ,'cashonpickup' ,'cielows' ,'spsbradesco' ,'pstransparenteloja5' ,'boleto' ,'edinar' ,'clictopay' ,'allpay' ,'pay2go' ,'cash' ,'postfinance' ,'pagseguro' ,'bcash' ,'braintreejs' ,'invoicepayment' ,'przelewy24' ,'prestalia_cashondelivery' ,'virtpaypayment' ,'cashondeliveryfeeplus' ,'pagonlineimprese' ,'mokejimai' ,'payplug' ,'seurcashondelivery' ,'universalpay' ,'mandiri' ,'bni' ,'bca' ,'veritranspay' ,'przelewy24' ,'transbancaria' ,'cashondeliveryplusmax' ,'multibanco' ,'ceca' ,'dotpay' ,'pagofacil' ,'postepay' ,'paysera' ,'offlinecreditcard' ,'paypaladvanced' ,'trustly' ,'cheque' ,'deluxecodfees'])!=-1) {
        if ($(dialogContainer + 'form').length) {
