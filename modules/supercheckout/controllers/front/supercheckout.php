...
class SupercheckoutSupercheckoutModuleFrontController extends ModuleFrontController
{
            ...
            if (Tools::isSubmit($this->name . 'PlaceOrder')) {
                ...
                switch (Tools::getValue('method')) {
                    ...
                    case 'loadInvoiceAddress':
                        $this->json = $this->loadInvoiceAddress(Tools::getValue('id_country'), Tools::getValue('id_state'), Tools::getValue('postcode'), Tools::getValue('city'), Tools::getValue('id_address_invoice'));
                        break;
                    case 'loadCarriers':
                        $this->json = $this->loadCarriers(Tools::getValue('id_country'), Tools::getValue('id_state'), Tools::getValue('postcode'), Tools::getValue('city'), Tools::getValue('id_address_delivery'));
                        break;
                    case 'setSameInvoice':
                    ...
                    case 'updateCarrier':
                        ...
                        break;
                    case 'loadPayment':
                            $this->json = $this->loadPaymentMethods(Tools::getValue('id_country'), Tools::getValue('id_address_delivery'), Tools::getValue('selected_payment_method_id'));
                        break;
        ...
        foreach ($result_array as $array_id) {
            ...
            if ($type == 'checkbox' && !isset($custom_field_values["field_$id_velsof_supercheckout_custom_fields"]) || ($type == 'radio' && !isset($custom_field_values["field_$id_velsof_supercheckout_custom_fields"]))) {
                ...
                if ($required == 1) {
                    ...
                    $errors['error']["field_$id_velsof_supercheckout_custom_fields"] = $this->module->l('Required Field');
                    return $errors;
                }
            }
            if ($flag_read == 1) {
                ...
                if ($validation_type != '0') {
                    if (!Validate::$validation_type($field_value)) {
                        ...
                        if ($validation_type == 'isName') {
                            $message = "Only name allowed.";
                        }
                        if ($validation_type == 'isEmail') {
                            $message = "Only email allowed (example@example.com).";
                        }
                        if ($validation_type == 'isDate') {
                            $message = "Only date is allowed. Check prestashop isDate() for reference.";
                        }
                        ...
                        $errors['error_occured'] = 1;
                        $errors['error']["field_$id_velsof_supercheckout_custom_fields"] = "Please provide data in valid format. $message";
                    }
                }
            }
        }
    ...
    public function initContent()
    {
        ...
        if (!$this->context->cart->nbProducts()) {
            ...
            if (isset($this->context->cookie->velsof_error) && $this->context->cookie->velsof_error) {
                $velsof_errors = Tools::unserialize($this->context->cookie->velsof_error);
    ...
    private function processSubmitLogin()
    {
        ...
        if (empty($this->json['error'])) {
            ...
            if (!$authentication || !$customer->id) {
                ...
                $this->json['success'] = $this->context->link->getModuleLink('supercheckout', 'supercheckout', array(), (bool) Configuration::get('PS_SSL_ENABLED'));
            }
        }
    }
    ...
    private function loadCarriers($id_country = 0, $id_state = 0, $postcode = '', $city = '', $id_address_delivery = 0, $default_carrier = null)
    {
        ...
        if ($this->context->cart->isVirtualCart()) {
            ...
            if ((!$this->is_logged || $this->is_logged) && empty($id_address_delivery)) {
                ...
                //Added to fix empty alias(500 error) issue on resetting database of psdemo. This ensures that cookie id has some assosiated address in database.
        ...
        if (Validate::isLoadedObject($delivery_address) && count($this->shipping_error) == 0) {
            ...
            } else if (!Validate::isLoadedObject($delivery_address) || $delivery_address->deleted) {
                $this->shipping_error[] = $this->module->l('No Delivery Method Available for this Address');
            }
            ...
            if (!count($this->shipping_error)) {
                ...
                $deliverdata = Tools::unserialize(Configuration::get('VELOCITY_SUPERCHECKOUT_DATA'));
                            ...
                            foreach ($deliverdata['delivery_method'] as $did => $deliveryid) {
                                if ($did == $carrier['instance']->id) {
                                    if ($deliveryid['logo']['title'] != '') {
                                        ...
                                        if ((bool) Configuration::get('PS_SSL_ENABLED') && $custom_ssl_var == 1) {
                                            $delivery_logo_path = _PS_BASE_URL_SSL_ . __PS_BASE_URI__ . 'modules/supercheckout/views/img/admin/uploads/' . $deliveryid['logo']['title'];
                                        } else {
                ...
                if (!empty($default_carrier) && isset($delivery_option_list[$id_address_delivery]) && array_key_exists($default_carrier . ',', $delivery_option_list[$id_address_delivery])) {
        ...
        if (count($address_without_carriers) && !$this->context->cart->isVirtualCart()) {
            if (count($address_without_carriers) > 1) {
                $this->shipping_error[] = $this->module->l('No Delivery Method Available for this Address');
            } else if ($this->context->cart->isMultiAddressDelivery()) {
                $this->shipping_error[] = $this->module->l('No Delivery Method Available for this Address');
            } else {
                $this->shipping_error[] = $this->module->l('No Delivery Method Available for this Address');
            }
    ...
    private function updateCarrier()
    {
        ...
        } else if (Tools::getIsset('id_carrier')) {
            $delivery_option_list = $this->context->cart->getDeliveryOptionList();
    ...
    protected function loadPaymentMethods($id_country = 0, $id_address_delivery = 0, $selected_payment_method = 0)
    {
        ...
        if ($methods) {
            foreach ($methods as $module) {
                ...
                if (($module_instance = Module::getInstanceByName($module['name'])) && ( is_callable(array($module_instance, 'hookpayment')) || is_callable(array($module_instance, 'hookDisplayPayment')))) {
                    $currency_ids = array();
                        ...
                        if (is_callable(array($module_instance, 'hookPayment')) || is_callable(array($module_instance, 'hookDisplayPayment'))) {
                            ...
                            if ($module['name'] == 'twenga') {
                                ...
                                if ($module['name'] == 'brinkscheckout') {
                                    ...
                                    $this->context->smarty->assign('twocheckout_sid', Configuration::get('TWOCHECKOUT_SID'));
                                    $this->context->smarty->assign('twocheckout_public_key', Configuration::get('TWOCHECKOUT_PUBLIC'));
                                }
                            }
...
//								. 'Pay by Credit Card or Paypal</a></p>';
                            ...
                            foreach ($matches as $match) {
                                ...
                                if ($module['name'] == 'mollie') {
                                    preg_match_all('/<a\s+(.*)href=\"(.*?)\"/i', $match[0], $url_matches, PREG_SET_ORDER);
                                    $url_query = parse_url($url_matches[0][2], PHP_URL_QUERY);
                                    parse_str($url_query, $method_name);
                                    $additional_payment_methods[$module_instance->id . '_' . $payment_number]['url'] = trim($url_matches[0][2]);
                                    $additional_payment_methods[$module_instance->id . '_' . $payment_number]['name'] = 'mollie_' . trim($method_name['method']);
                                }
                                $payment_number++;
                            }
                        }
                        ...
                        if ($is_hook && !empty($html)) {
                            ...
                            if (($module['name'] == 'zipcheck' && $this->is_logged && $payment_module_url == '') || ($module['name'] == 'zipcheck' && $payment_module_url == '' && Tools::isSubmit('ajax'))) {
                                continue;
                            }
        ...
        if ($counter == 0) {  // added for ship2pay
            $payment_methods['not_required'] = $this->module->l(
                'No payment method is available for your selected delivery method.'
            );
        } else {
                ...
                foreach ($paymentdata['payment_method'] as $key => $payid) {
                    if ($payment_methods['methods'][$payment_id]['id_module'] == $key) {
                        if ($payid['logo']['title'] != '') {
                            ...
                            $temp = $values;
                            unset($temp);
                            $logo_path = 'modules/supercheckout/views/img/admin/uploads/' . $payid['logo']['title'];
                            $pay_path = _PS_ROOT_DIR_ . '/' . $logo_path;
                            if (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] == 'on') {
                                $custom_ssl_var = 1;
                            }
                            if ((bool) Configuration::get('PS_SSL_ENABLED') && $custom_ssl_var == 1) {
                                $logo_path = _PS_BASE_URL_SSL_ . __PS_BASE_URI__ . $logo_path;
                            } else {
                                $logo_path = _PS_BASE_URL_ . __PS_BASE_URI__ . $logo_path;
                            }
                            if (file_exists($pay_path)) {
                                $payment_methods['methods'][$payment_id]['payment_image_url'] = $logo_path;
                                $p_w = $payid['logo']['resolution']['width'];
                                $payment_methods['methods'][$payment_id]['width'] = $p_w;
                                $p_h = $payid['logo']['resolution']['height'];
                                $payment_methods['methods'][$payment_id]['height'] = $p_h;
                            }
                        }
                        if ($payid['logo']['resolution']['width'] != 'auto') {
                            $payment_methods['methods'][$payment_id]['width'] = $payid['logo']['resolution']['width'];
                        }
                        if ($payid['logo']['resolution']['height'] != 'auto') {
    ...
    public function getPaymentInformation($payment_module_name)
    {
        ...
        $hook_args = array('cookie' => $this->context->cookie, 'cart' => $this->context->cart);
        if (($module_instance = Module::getInstanceByName($payment_module_name)) && ( is_callable(array($module_instance, 'hookpayment')) || is_callable(array($module_instance, 'hookDisplayPayment')))) {
            if (!$module_instance->currencies || ( $module_instance->currencies && count(Currency::checkPaymentCurrencies($module_instance->id)))) {
                if (is_callable(array($module_instance, 'hookPayment')) || is_callable(array($module_instance, 'hookDisplayPayment'))) {
                    $is_hook = false;
                    $html = '';
...
//					$null, true, true, $cart_product_context);
            ...
            if (!($code = trim(Tools::getValue('discount_name')))) {
                ...
                if (($cart_rule = new CartRule(CartRule::getIdByCode($code))) && Validate::isLoadedObject($cart_rule)) {
                    if ($error = $cart_rule->checkValidity($this->context, false, true)) {
                        ...
                        $summary = $this->context->cart->getSummaryDetails(null, true);
                        if (!empty($summary['gift_products'])) {
                            $discountarr['refresh'] = 1;
                        }
    ...
    private function confirmOrder()
    {
        ...
        if (Tools::getIsset($_POST['use_for_invoice'])) {
        ...
        if ($this->supercheckout_settings['confirm']['term_condition'][($this->is_logged) ? 'logged' : 'guest']['require'] == 1) {
            if (!isset($posted_data['cgv']) || $posted_data['cgv'] != 1) {
                $response['error']['general'][] = $this->module->l('Please acccept our terms & conditions before confirming your order');
                return $response;
            }
        }
        ...
        if ($this->nb_products > 0) {
            ...
            if ((isset($posted_data['shipping_address_value']) && $posted_data['shipping_address_value'] == 1) || !isset($posted_data['shipping_address_value'])) {
                if (isset($this->context->cookie->supercheckout_temp_address_delivery) && $this->context->cookie->supercheckout_temp_address_delivery > 0) {
                    $id_delivery_address = $this->context->cookie->supercheckout_temp_address_delivery;
                }
            } else if (isset($posted_data['shipping_address_value']) && $posted_data['shipping_address_value'] == 0 && isset($posted_data['shipping_address_id'])) {
            ...
            } else if (((isset($posted_data['payment_address_value']) && $posted_data['payment_address_value'] == 1) || !isset($posted_data['payment_address_value']))) {
                if (isset($this->context->cookie->supercheckout_temp_address_invoice) && $this->context->cookie->supercheckout_temp_address_invoice > 0) {
            ...
            } else if (!isset($posted_data['use_for_invoice']) && isset($posted_data['payment_address_value']) && $posted_data['payment_address_value'] == 0) {
            ...
            if (!$this->is_logged) {
                ...
                foreach ($posted_data['customer_personal'] as $key => $value) {
                    if ($key != 'dob_days' && $key != 'dob_months' && $key != 'dob_years') {
                        if ($key == 'password') {
                            ...
                            if (isset($this->supercheckout_settings['customer_personal'][$key][$user_type]['require']) && $this->supercheckout_settings['customer_personal'][$key][$user_type]['require'] == 1 && !isset($posted_data['customer_personal'][$key])) {
                                $response['error']['customer_personal'][] = array('key' => $key,
                ...
                $check_dob = false;
                if (isset($posted_data['customer_personal']['dob_days']) && isset($posted_data['customer_personal']['dob_months']) && isset($posted_data['customer_personal']['dob_years'])) {
            ...
            if ($shipping_address_value == 1) {
                foreach ($posted_data['shipping_address'] as $key => $value) {
                    ...
                    if (($key == 'phone_mobile' || $key == 'phone') && !empty($posted_data['shipping_address'][$key]) && !(boolean) Validate::isPhoneNumber($posted_data['shipping_address'][$key])) {
                    ...
                    if ($key == 'id_country') {
                        ...
                        } else if ((int) $country->contains_states && (isset($posted_data['shipping_address']['id_state']) && !(int) $posted_data['shipping_address']['id_state'])) {
                            $response['error']['shipping_address'][$loop_index] = array('key' => $key,
                                'error' => $this->module->l('This country requires you to chose a State'));
                        }

                        if (isset($posted_data['shipping_address']['postcode']) && $postcode_error = $this->checkZipForCountry($country, $posted_data['shipping_address']['postcode'], true)) {
                        ...
                        if ($this->supercheckout_settings['shipping_address']['dni'][$user_type]['require'] == 1 && $country->isNeedDni()) {
                            if (isset($posted_data['shipping_address']['dni']) && $country->isNeedDni() && ($posted_data['shipping_address']['dni'] == '' || !Validate::isDniLite($posted_data['shipping_address']['dni']))) {
            ...
            if (!isset($posted_data['use_for_invoice'])) {
                ...
                if ($payment_address_value == 1) {
                    foreach ($posted_data['payment_address'] as $key => $value) {
                        ...
                        if ($add_plugin_config['conditional'] == 0 && $add_plugin_config[$user_type]['require'] == 1 && $posted_data['payment_address'][$key] == '') {
                            $response['error']['payment_address'][$loop_index] = array('key' => $key,
                                'error' => $this->module->l('Required Field'));
                        }
                        if (($key == 'phone_mobile' || $key == 'phone') && !empty($posted_data['payment_address'][$key]) && !(boolean) Validate::isPhoneNumber($posted_data['payment_address'][$key])) {
                            $response['error']['payment_address'][$loop_index] = array('key' => $key,
                        ...
                        if ($key == 'id_country') {
                            ...
                            } else if ((int) $country->contains_states && (isset($posted_data['payment_address']['id_state']) && !(int) $posted_data['payment_address']['id_state'])) {
                                $response['error']['payment_address'][$loop_index] = array('key' => $key,
                                    'error' => $this->module->l('This country requires you to chose a State'));
                            } else if (!$country->active) {
                                $response['error']['payment_address'][$loop_index] = array('key' => $key,
                            ...
                            if (isset($posted_data['payment_address']['postcode']) && $postcode_error = $this->checkZipForCountry($country, $posted_data['payment_address']['postcode'])) {
                            ...
                            if (($this->supercheckout_settings['payment_address']['dni'][$user_type]['require'] == 1) && $country->isNeedDni()) {
                                if (isset($posted_data['payment_address']['dni']) && $country->isNeedDni() && ($posted_data['payment_address']['dni'] == '' || !Validate::isDniLite($posted_data['payment_address']['dni']))) {
            ...
            if ((isset($posted_data['shipping_address_value']) && $posted_data['shipping_address_value'] == 1) || !isset($posted_data['shipping_address_value'])) {
                ...
                $delivery_address->alias = (isset($posted_data['shipping_address']['alias'])) ? (empty($posted_data['shipping_address']['alias'])) ? $this->module->l('Title Delivery Alias') . ' - ' . date('s') . rand(0, 9) : $posted_data['shipping_address']['alias'] : $this->module->l('Title Delivery Alias') . ' - ' . date('s') . rand(0, 9);
            ...
            if (isset($posted_data['use_for_invoice']) && ((isset($posted_data['shipping_address_value']) && $posted_data['shipping_address_value'] == 1) || !isset($posted_data['shipping_address_value']))) {
                $invoice_address = $delivery_address;
                $id_invoice_address = $id_delivery_address;
            } else if (isset($posted_data['use_for_invoice']) && isset($posted_data['shipping_address_value']) && $posted_data['shipping_address_value'] == 0) {
                $id_invoice_address = $id_delivery_address;
            }

            if (!isset($posted_data['use_for_invoice']) && ((isset($posted_data['payment_address_value']) && $posted_data['payment_address_value'] == 1) || !isset($posted_data['payment_address_value']))) {
                ...
                $invoice_address->alias = (isset($posted_data['payment_address']['alias'])) ? (empty($posted_data['payment_address']['alias'])) ? $this->module->l('Title Delivery Alias') . ' - ' . date('s') . rand(0, 9) : $posted_data['payment_address']['alias'] : $this->module->l('Title Delivery Alias') . ' - ' . date('s') . rand(0, 9);
...
//							? trim($delivery_option[$id_delivery_address], ',') : 0;
        ...
        //$custom_fields_response = $this->validateCustomFieldsData($custom_field_values);
        ...
        if (isset($custom_fields_response['error_occured']) && $custom_fields_response['error_occured'] == 1) {
            $response['error']['general'][] = $this->module->l('Please provide required information. Check all the fields in the form.');
            $response['custom_fields_errors'] = $custom_fields_response;
            return $response;
        }
        //Rest of the validations are performed only if the custom fields are filled properly
        if (!isset($response['error'])) {
    ...
    private function getFormattedAddress()
    {
        ...
        if (Validate::isLoadedObject($customer)) {
            ...
            foreach ($customer_addresses as $address) {
                ...
                $formated_adrress_values_list[$address['id_address']]['formated_fields_values'] = AddressFormat::getFormattedAddressFieldsValues($tmp_address, $formated_adrress_values_list[$address['id_address']]['ordered_fields']);
