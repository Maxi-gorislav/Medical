...
{counter}
<div id="{$role|escape:'htmlall':'UTF-8'}-{$count_rule|intval}" class="panel-collapse collapse{if $count_rule == 1} in{/if}">
	<div class="table-responsive clearfix">
		<table id="table-{$role|escape:'htmlall':'UTF-8'}-{$count_rule|intval}" cellpadding="0" cellspacing="0" border="0" class="datatable table table-striped table-bordered table-hover dataTableHidden">
			<thead>
			<tr>
				<th class="text-center">{l s='Country' mod='gshopping'}</th>
				<th class="text-center">{l s='ID Category' mod='gshopping'}</th>
				<th class="text-center">{l s='Shop' mod='gshopping'}</th>
				<th class="text-center">{l s='Name' mod='gshopping'}</th>
				<th class="text-center">{l s='Status' mod='gshopping'}</th>
				<th class="text-center">{l s='Actions' mod='gshopping'}</th>
			</tr>
			</thead>
			<tbody>
					...
					*}
					<td class="fixed-width-sm text-center">
						{if $prod.nb_obj|escape:'htmlall':'UTF-8' == 'All'}
							{l s='All categories' mod='gshopping'}
						{else}
					...
					<td class="pointer fixed-width-sm text-center">
						...
						<a class="list-action-enable action-{if $prod.active|intval == 1}enabled{else}disabled{/if}" title="{if $prod.active|intval == 1}{l s='Enabled' mod='gshopping'}{else}{l s='Disabled' mod='gshopping'}{/if}">
