...
<script type="text/javascript"> 
    $(document).ready(function() {    $("#lightSlider").lightSlider({        item: 1,        mode:"fade",        auto:true,        loop:true    });   });
</script>
{if $teste_scrolling==1}      <div class="testimonialHolder block col-xs-12 col-md-8 col-sm-8" > 	      
        {if $test_titles}                   {*$test_titles|print_r*}		 
                <ul id="lightSlider">                  		
                    {foreach from=$test_titles item=testmonial name=test_title}   
                        <li>			<!-- HTML codes by Quackit.com -->	
                    ...
                    {if $this_version == '1.6'}	
                        <a href="index.php?fc=module&module=testimonial&controller=testi_list">
                            {l s='Know more..' mod='testimonial'} </a>		
                    {else if $this_version == '1.5'}		
                        <a href="index.php?fc=module&module=testimonial&controller=testi_list">
                            {l s='Know more..' mod='testimonial'} </a>		
                    {else if $this_version == '1.4'}		
                    ...
                    {/if}	    
            {else} <a href="index.php?fc=module&module=testimonial&controller=testi_list">
                    {l s='Know more..' mod='testimonial'} </a>	     
            {/if}	         
                ...
                <div class="block_content" style="padding:10px; overflow-x:auto;">	
                    {if $test_titles}		
                        ...
                        {foreach from=$test_titles item=testmonial name=test_title}	
                            ...
                            {else if $this_version == '1.4'}	   
                                <a href="{$this_path}testi_list.php">{l s='Know more..' mod='testimonial'} </a>	
                            {/if}		  {else} {l s='No Testimonial..' mod='testimonial'}	
                            {/if}	   </div>      </div>   {/if} 
