...
class CommentNewsClass extends ObjectModel
{
	...
	public function deleteTablesBdd()
	{
		if (!Db::getInstance(_PS_USE_SQL_SLAVE_)->Execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.$this->table))
			return false;
		if (!Db::getInstance(_PS_USE_SQL_SLAVE_)->Execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.$this->table.'_abo'))
			return false;
	...
	public static function getNbCommentsFromFacebook($url)
	{
		$api = Tools::file_get_contents('https://graph.facebook.com/v2.4/?fields=share{comment_count}&id='.urlencode($url));
		$comments = Tools::jsonDecode($api);
		return (int)$comments->share->comment_count;
	}

	/*
	public static function getUserInfosNameFromFacebookId($facebook_id)
	{
		$api = Tools::file_get_contents('http://graph.facebook.com/'.(int)$facebook_id);
		$user_infos = Tools::jsonDecode($api);
